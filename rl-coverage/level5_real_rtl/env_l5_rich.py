"""Same as env_l5.IbexL5Env but with a rich observation that tells PPO which
modules still have uncovered toggle points.

Per-step observation (30 dims):
  [0]  step_idx / episode_steps          (progress within episode)
  [1]  n_episodes / 100                  (how long we've been training)
  [2]  overall_cum_toggle_pct            (global coverage so far)
  [3 : 3 + N_MODULES]                    per-module covered fraction
                                         (lets the policy target undercovered modules)
"""
...
# The modules we care about, in a fixed order so the observation is consistent
# across episodes. Derived from the pages we saw in coverage.dat.
MODULES = [
...
def _module_coverage(summary: cov_parser.CovSummary) -> np.ndarray:
    """Return a vector of per-module toggle-covered fractions (0..1)."""
    out = np.zeros(N_MODULES, dtype=np.float32)
    # Sum toggle points per module across the subpages (e.g. ibex_core__pi3).
    cov = defaultdict(lambda: [0, 0])  # module -> [covered, total]
...
class IbexL5RichEnv(gym.Env):
    ...
    def __init__(self, episode_steps: int = 1024, seed: int | None = None,
        ...
        self._n_episodes = 0
        self._ep_total = 20023  # filled after first episode
